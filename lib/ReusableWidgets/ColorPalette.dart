import 'package:flutter/material.dart';

class ColorPalette {
  Color black = Colors.black;
  Color white = Colors.white;


  Color dolorPosoperatorioBackground = const Color(0xffEFF2E1);
  Color dolorPosoperatorioRojoCabecera = const Color(0xffCA3539);
  Color dolorPosoperatorioRojoSevero = const Color(0xffB21D0D);
  Color dolorPosoperatorioAzulRey = const Color(0xff3F4096);
  Color dolorPosoperatorioAzul = const Color(0xff0666A1);
  Color dolorPosoperatorioAzulClaro = const Color(0xff60BAE9);
  Color dolorPosoperatorioGreen = const Color(0xff82A74B);
  Color dolorPosoperatorioGreenSinDolor = const Color(0xff1CB262);
  Color dolorPosoperatorioOrange = const Color(0xffF59108);

  Color enfermedadTromboembolicaVenosaBackground = const Color(0xffFEF0F3);
  Color enfermedadTromboembolicaVenosaRojoCabecera = const Color(0xffAE3340);
  Color enfermedadTromboembolicaVenosaRosado = const Color(0xffF7A188);
  Color enfermedadTromboembolicaVenosaNaranja = const Color(0xffE84A38);
  Color enfermedadTromboembolicaVenosaRojo = const Color(0xffE93438);
  Color enfermedadTromboembolicaVenosaAmarillo = const Color(0xffF3BC32);

  Color tromboprofilaxisBackground = const Color(0xffE1EFFA);
  Color tromboprofilaxisGris = const Color(0xff737476);
  Color tromboprofilaxisAzulrey = const Color(0xff0666A1);
  Color tromboprofilaxisAzulClaro = const Color(0xff5DC7D1);
  Color tromboprofilaxisAzulClaroIntenso = const Color(0xff00B0F0);

  Color infeccionTractoUrinarioVerde = const Color(0xff52658C);
  Color infeccionTractoUrinarioAmarillo = const Color(0xffF3BC32);
  Color infeccionTractoUrinarioVerdeOscuro = const Color(0xff52658C);
}